...
```
    ...
    a. 第1个 static 表示当前函数是私有的, 被包含的文件所有, 作用域是具体文件中这个函数
       以下范围.
    b. 第2个 static [static 64] 是 C99 出现的语法, 告诉编译器这个"数组"至少有 64 个
       数据单元, 您可以放心优化.
    c. 第3个 static 表示声明一个私有静态变量 cnt, 生存周期属于 heoo 函数域. 
    d. 另一个细节是, 生命周期是未初始化全局的变量. 默认内存都是以0填充的, = 0 是缺省的.

> 科普小细节 C 中 0, 00, 0x0, .0, '\0', '0', "0", NULL, false, EXIT_SUCCESS 什么鬼!  
