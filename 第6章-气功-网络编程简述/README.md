...
文末我们会简单讲解网络库要处理那些业务他的来由. 全章流程迂回递进, 需要反复回溯, 必有豁然开朗醍醐灌顶. 修真, 修炼, 一个不传的隐秘就是, 有了持续时间精进和反思感悟就会拥有自己的全世界. 例如 <<无尽剑装>> 小说主角结局最终靠寿元兑换大道感悟 $ 补充之前修炼的经验, 当初刚做开发时候. 公司朴实的前辈说, 搞个 TCP 估计 1 年就过去了. 后来想想还真是, 虽然到现在还是水经验. 但有一点可以确定, 编程, 修炼并不是一个青春饭. 很需要底蕴和积累. 投机倒把不能干成一件需要长时间心力体力投入的事情. 大佬赠我越努力越幸运, 算个小确幸送给修真路上的道友们. 当有一天, 打游戏了累了, 刚好没目标. 那就打开笔记本, 秀出你的神之一技吧 ❤ 山中不知岁月, 一切才刚刚开始 ~
...
char 多数平台是 1 字节大小, 8bit. 多数环境(CL or GCC)中是有符号 signed 的 (ARM 无符号 unsigned), 范围是[-128, 127]. 而在工程项目开发中推荐用 int8_t 和 uint8_t 替代 char.
...
程序开发常遇到的就是自解释问题, 鸡生蛋, 蛋生鸡. 后面再分析 signed 和 unsigned.     
...
short 多数占 2 字节, 为有符号的, 默认自带 signed. 范围 [-2^15, 2^15 - 1] 2^15 = 32800. 推荐使用 int16_t or uint16_t 类型.
...
int 声明的变量, 多数占 4 字节, 有符号 signed. 范围 [-2^31, 2^31 - 1]. 相似的有 int32_t 和 uint32_t 类型.
...
被修饰的变量声明为无符号类型. 范围 >= 0.  unsigned 只能修饰整型的变量. 当然当你用这个修饰变量的时候. 再使用 - 和 -- 运算的时候一定要小心, 存在数据值变的很大, 超出预期可能.
...
我这里对于 select 介绍是百不存一, 后续强烈推荐读者认真演技相关资料. 他如此与众不同. 到这里本节应该结束了, 但是一念又想起高中老师的谆谆教导, 回赠个总的复习吧!
...
可能未来会统一使用 IPv6 地址协议, 也可能是更先进的地址协议, 或者出现大一统的完备系数高的语言. 但终究以后的编码创造会更加简单. 随后将会详细剖析其中接口, 这些技能是服务器开发工程师升级的潜在财富. 一行 socket 没写过, 却能解决亿万并发, 分布式缓存, 那应届生应该也可以吧. socket 他就是练气期大圆满的里程碑. 
